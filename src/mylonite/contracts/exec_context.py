"""Execution-context metadata carried alongside an exploit.

**Root cause this closes (T12, promoted in 0.7.10).**
``TestGenerator.emit(self, exploit)`` used to only ever receive an
:class:`~mylonite.contracts._types.ExploitRecord`, which has no model/provider
field -- those live on :class:`~mylonite.contracts._types.ScanReport`, a
SIBLING artefact ``emit`` never read. Left alone, the emitted test's runtime
helpers (:mod:`mylonite.testkit`) fell back to a hardcoded model/provider
default -- meaning a committed regression test could silently gate CI using a
DIFFERENT model than the one that actually found/validated the exploit.

T12 (0.7.8) threaded :class:`ExecContext` through ``Payload.metadata: dict[str,
str]`` as an interim compatibility shim -- a field whose JSON schema is
already ``{"additionalProperties": {"type": "string"}}``, so it needed no
schema change and no `contract-change` clock. Every key it writes is
namespaced under :data:`METADATA_PREFIX` (``"mylonite.exec."``), a prefix
RESERVED for this purpose via `contract-change`-tagged issue #78.

0.7.10 promotes that shim into a real ``TestGenerator.emit(exploit,
context=None)`` parameter (see ``contracts/test_generator.py``,
``CONTRACT_VERSION`` 0.1.0 -> 0.2.0). ``ExecContext`` itself now lives HERE,
under ``contracts/``, rather than under ``scan/`` where T12 originally placed
it: ``contracts/test_generator.py`` needs a real type for the new parameter,
and ``contracts/`` importing FROM ``scan/`` would invert this project's
layering (``scan/`` is a consumer of the contracts, not the other way
around). :mod:`mylonite.scan.exec_context` re-exports every name from this
module unchanged, so existing ``from mylonite.scan.exec_context import
ExecContext`` call sites keep working.

**One writer, several readers, allowlisted.**
...
* Readers: :meth:`mylonite.plugins._reference.reference_pytest_generator.ReferencePytestGenerator.emit`
  now receives an explicit ``context: ExecContext | None`` parameter and
  renders its ``model=``/``provider=`` straight into the generated test
  source -- falling back to :meth:`ExecContext.from_metadata` only when no
  context is passed (a pre-migration caller, or a pre-T12 exploit with no
  exec context stamped at all). :mod:`mylonite.testkit`'s
  ``assert_target_resists``/``assert_control_holds`` separately read the
  metadata as one step of their OWN runtime resolution order (explicit kwarg
  -> exec context -> sibling ``scan_report.json`` back-fill -> loud failure)
  -- a distinct, run-time concern from ``emit``'s generation-time one.
...
#: Reservation tracked by a `contract-change`-tagged GitHub issue (#78, see T12
#: in the 0.7.7->0.7.10 remediation plan) -- a plugin author must not stamp
#: keys under this prefix for anything other than :class:`ExecContext`.
