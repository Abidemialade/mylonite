...
_TEMPLATE_SYNTHESIZED = '''"""Auto-generated by mylonite — security regression test for the synthesized \
tool-chain `{pattern_id}` against `{target_id}`.
...
@pytest.mark.skipif(
    os.environ.get("MYLONITE_LIVE_TARGET") != "1",
    reason="re-drives the synthesized chain live; set MYLONITE_LIVE_TARGET=1 to run",
)
def test_security_{slug}() -> None:
    """{target_id} must keep resisting the synthesized chain {pattern_id} \
({compliance_inline})."""
    here = Path(__file__).parent
    exploit = testkit.load_exploit(here / "exploit_{pattern_id}.json")
    testkit.assert_synthesized_chain_resists(exploit)
'''
...
class ReferencePytestGenerator(TestGeneratorBase):
    ...
    def emit(self, exploit: ExploitRecord) -> GeneratedTest:
        ...
        if "synthesized_chain" in exploit.payload.metadata:
            template = _TEMPLATE_SYNTHESIZED
        elif "synthetic_control" in exploit.payload.metadata:
