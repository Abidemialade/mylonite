...
def test_resolves_relative_path_inside_base(tmp_path) -> None:
...
def test_rejects_absolute_path_outside_base(tmp_path) -> None:
    with pytest.raises(PathEscapesBase):
        resolve_contained("/etc/passwd", base=tmp_path, label="system_prompt_file")


def test_rejects_symlink_pointing_outside_base(tmp_path) -> None:
