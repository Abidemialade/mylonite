...
def test_emit_escapes_target_id_that_would_break_the_docstring() -> None:
    """DCR-0006: `target_id` was interpolated bare into every emitted docstring
    (module docstring in every template, plus the function docstring of
    `_TEMPLATE_CUSTOM` / `_TEMPLATE_CONTROL`) -- unlike `pattern_id` (validated
    via `_SAFE_PATTERN_ID`) or `control`/`model`/`provider` (rendered via
    `_py_literal` at CODE sites, `_slugify` at DOCSTRING sites). `TargetFile.family`
    (see `plugins/_mcp/target_file.py`) is a bare `str` with only a 3-reserved-name
    validator, so an operator or MCP server can supply almost anything into the
    `target_id` (`mcp:<family>:<scope>`) this generator embeds. A value containing
    `\"\"\"` terminates the docstring early and the remainder becomes live,
    executable top-level code the moment pytest collects the emitted, committed
    test file.

    The hostile value below starts with `reference:` so `emit()` selects
    `_TEMPLATE` (the only template with no legitimate top-level `import os` of
    its own), forcing this to genuinely prove the escaping -- not merely dodge
    ...
    assert "system" not in top_level_calls


def test_emit_escapes_target_id_in_custom_and_control_templates() -> None:
    """Same DCR-0006 property for the CUSTOM/CONTROL templates, which ALSO
    interpolate `target_id` into the per-function docstring (not just the
    module docstring) -- and both templates legitimately `import os` already,
    so this proves the injected statement doesn't become a NEW top-level
...
_GOLDEN = '''\
"""Auto-generated by mylonite — security regression test for `indirect-injection-note-body-direct` against `reference_vulnerable`.
...
def test_custom_target_emits_real_target_assertion() -> None:
    ...
    # DCR-0006: target_id is docstring-slugified (`:` -> `_`), like control_display.
    assert "mcp_acme" in source
